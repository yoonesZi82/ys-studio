...
export const AnimatedSpan = ({
  ...
  useEffect(() => {
    ...
    if (sequence?.activeIndex === itemIndex) {
      timeoutId = setTimeout(() => setHasStarted(true), 0);
    }
...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "border-border bg-background z-0 h-full overflow-hidden max-h-100 w-full max-w-lg rounded-xl border",
        className,
      )}
    >
      <div className="border-border bg-muted/20 flex flex-col gap-y-2 border-b p-4">
      ...
      <pre className="p-4 bg-background">
        <code className="grid gap-y-1 overflow-auto scrollbar-hide">
          {wrappedChildren}
        </code>
      </pre>
    </div>
